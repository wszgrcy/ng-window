...
import {
  ...
  ToastPosition,
  HintOptions,
  HintOptionsWithPosition,
} from "src/interface/toast.interface";
...
export class ToastService {
  ...
  ) // private viewContainerRef: ViewContainerRef
  {
    ...
    this.container.classList.add(
      `h-${this.position.horizontalPosition}`,
      `v-${this.position.verticalPosition}`
    );
  }
  ...
  setPosition(toastPosition: ToastPosition) {
    ...
    this.container.classList.add(
      `h-${toastPosition.horizontalPosition}`,
      `v-${toastPosition.verticalPosition}`
    );
