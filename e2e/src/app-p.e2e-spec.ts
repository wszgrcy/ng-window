...
describe('图标点击操作', () => {
  ...
  it('点击应用-最小化', async () => {
    const appList = await page.getAppList();
    ...
    for (let i = 0; i < appCount; i++) {
      ...
      const element = await page.findActiveWindow();
  ...
  it('点击应用-最大化', async () => {
    const appList = await page.getAppList();
    const appCount = await appList.length;
    for (let i = 0; i < appCount; i++) {
      /**图标 */
      const app = appList[i];
      console.log(i);
      ...
      await webPage.waitFor(500);
      // console.log('准备最大化')
      const componentWindow = new ComponentWIndowP(await page.findActiveWindow(), browser, webPage);
...
describe('任务栏操作', () => {
  ...
  it('移动任务栏', async () => {
    const { width, height } = await webPage.viewport();
