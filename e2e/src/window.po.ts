...
export class ComponentWIndow {
    ...
    async  max(restore?: boolean) {
        ...
        const maxButton = this.buttonListFinder.get(1);
    ...
    async min() {
        const minButton = this.buttonListFinder.get(0);
        return browser.actions()
    ...
    async close() {
        const minButton = this.buttonListFinder.get(2);
        return browser.actions()
            .mouseMove(await minButton.getWebElement())
            .click(await minButton.getWebElement())
            .perform();
     ...
     */
    async move({ x, y }) {
        const header = this.elementFinder.$('header title');
    ...
    async    adjustWindow(direction: string, size: number) {
        const el = this.elementFinder.$(`.resize.${direction}`);
        let x = 0, y = 0;
        switch (direction) {
            case 'left':
                x = -size;
