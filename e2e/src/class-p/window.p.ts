...
export class ComponentWIndowP extends ElementBase {
    ...
    async  max(restore?: boolean) {
        ...
        const maxButton = this.buttonListFinder[1];
        const pos = mousePositionCenter(await maxButton.boundingBox());
        return this.mouse.click(pos.x, pos.y);
    }
    async min() {
        const minButton = this.buttonListFinder[0];
        const pos = mousePositionCenter(await minButton.boundingBox());
        return this.mouse.click(pos.x, pos.y);
    }
    async close() {
        const minButton = this.buttonListFinder[2];
        const pos = mousePositionCenter(await minButton.boundingBox());
        return this.mouse.click(pos.x, pos.y);
    }
    /**
